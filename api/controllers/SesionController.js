...
module.exports = {
  ...
  procesarRegistro: async (peticion, respuesta) => {
    let cliente = await Client.findOne({ email: peticion.body.email });
    if (cliente) {
      peticion.addFlash("mensaje", "Email duplicado");
      return respuesta.redirect("/registro");
    } else {
      let cliente = await Client.create({
  ...
  procesarInicioSesion: async (peticion, respuesta) => {
    ...
    if (cliente) {
      if (!cliente.activo) {
        peticion.addFlash(
          "mensaje",
          "Su cuenta ha sido desactivada, contacte con un administrador para resolver este problema"
        );

...
module.exports = {
  agregarCarroCompra: async (peticion, respuesta) => {
    let foto = await CarroCompra.findOne({
    ...
    if (foto) {
      peticion.addFlash(
        "mensaje",
        "La foto ya había sido agregada al carro de compra"
      );
    } else {
      ...
      peticion.session.carroCompra = await CarroCompra.find({
        cliente: peticion.session.cliente.id,
      });
  ...
  carroCompra: async (peticion, respuesta) => {
    ...
    let elementos = await CarroCompra.find({
      cliente: peticion.session.cliente.id,
    }).populate("foto");
  ...
  ordenDeCompra: async (peticion, respuesta) => {
    ...
    orden.detalles = await OrdenDetalle.find({ orden: orden.id }).populate(
      "foto"
    );
    return respuesta.view("pages/orden", { orden });
  },
  ...
  agregarListaDeseo: async (peticion, respuesta) => {
    let foto = await ListaDeseo.findOne({
      foto: peticion.params.fotoId,
      cliente: peticion.session.cliente.id,
    });
    if (foto) {
      peticion.addFlash(
        "mensaje",
        "La foto ya había sido agregada a la lista de deseo"
      );
    } else {
  ...
  listaDeseo: async (peticion, respuesta) => {
    ...
    let elementos1 = await ListaDeseo.find({
      cliente: peticion.session.cliente.id,
    });
    console.log(elementos1);
    let elementos = await ListaDeseo.find({
      cliente: peticion.session.cliente.id,
    }).populate("foto");
